...
Σ-identity {A = A} {P} {w} {w'} = f , qinv→isequiv (g , α , β) where
  f : {w w' : Σ A P} → w == w' → Σ (pr₁ w == pr₁ w') λ p → transport p (pr₂ w) == pr₂ w'
  f p rewrite p = refl , refl
  g : {w  w' : Σ A P} → Σ (pr₁ w == pr₁ w') (λ p → transport p (pr₂ w) == pr₂ w') → w == w'
  g {w = w₁ , w₂} {w₁' , w₂'} (p , q) rewrite p | q = refl
  α : {w w' : Σ A P} → f {w} {w'} ∘ g ~ id
  α {w₁ , w₂} {w₁' , w₂'} (p , q) rewrite p | q = refl
  β : {w w' : Σ A P} → g {w} {w'} ∘ f ~ id
  β {w₁ , w₂} {w₁' , w₂'} refl = refl
